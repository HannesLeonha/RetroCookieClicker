...
def update():
    ...
    if shop.cpsFromItems * 0.1 <= 1:
        handler.update(shop.cpsFromItems, 1, cookie.checkCookiePressed(mousePos, mousePressed), golden.active, golden.cookieEffect)
    else:
        handler.update(shop.cpsFromItems, shop.cpsFromItems * 0.1, cookie.checkCookiePressed(mousePos, mousePressed), golden.active, golden.cookieEffect)
...
while running:
    ...
    if pg.key.get_pressed()[pg.K_ESCAPE] == 1 and DEVELOPMENT_MODE:
        running = False
